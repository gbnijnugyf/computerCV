...
def euclidean_distance_native(u, v):
    ...
    ### YOUR CODE HERE
    squared_sum = sum((p1 - p2) ** 2 for p1, p2 in zip(u, v))
...
def get_eigen_values_and_vectors(M, k):
    ...
    eigen_indices = np.argsort(eigenvalues)[::-1]  # Sort eigenvalues in descending order
    eigenvalues = eigenvalues[eigen_indices][:k]
    eigenvectors = eigenvectors[:, eigen_indices][:, :k]
    if k == 1:
